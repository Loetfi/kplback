...
class KonsumsiRapatController extends Controller
{ 
    public $title = 'Serba Usaha - Konsumsi Rapat';
    public $layanan = 'konsumsirapat';
	...
	{
		$r = (object) RestCurl::exec('GET',env('LINK_API').'data/orderlist?start=0&length=1000&id_kategori=11');
        $list =  @$r->data->data;
	...
	{ 
		...
		$res = (object) RestCurl::exec('GET',env('LINK_API').'/backend/toko/detailData?id_layanan=4&id_kategori=11&id_order='.$id);
        // dd($res);
		if ($res->status == 200) {
	...
	public function prosesedit($id_order = 0, $id = 0){
		...
		$data = array(
			'id_order' => $id_order,
			'status' => $id,
			'kategori' => 'Serba Usaha - Konsumsi Rapat'
		);
